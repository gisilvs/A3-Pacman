...
class NNAgent(CaptureAgent):
    ...
    def registerInitialState(self, gameState):
        ...
        if load == 1:
            try:
                self.policy_net = torch.load('/output/policy_net_h')
            except:
                print('COULDNT LOAD')
        ...
        if load == 1:
            try:
                open("/output/memo_h.file", "rb")
                with open("/output/memo_h.file", "rb") as f:
    ...
    def finalUpdate(self, winner, gameState):
        ...
        self.memory.push(torch.from_numpy(self.old_state).unsqueeze(0).type(Tensor), LongTensor([[self.old_action]]),
                    torch.from_numpy(state).unsqueeze(0).type(Tensor), reward)
        self.optimize_model()
        torch.save(self.policy_net, '/output/policy_net_h')
        with open("/output/memo_h.file", "wb") as f:
